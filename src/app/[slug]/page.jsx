 ...
 * - No footer / author box / related posts
 * - Overflow-x removed
 * - Only first gallery block rendered; gallery is large and centered
 * - Persian date shown under article
 * - TOC on the right (anchors to headings)
 * - Blocks supported: title, text, html, image, gallery, video, embed, link, code
 * - generateMetadata + JSON-LD included
 */
...
function Block({ b, i, pageTitle, galleryPrefix, renderGallery }) {
  ...
  if (type === "gallery" && Array.isArray(b.value)) {
    if (!renderGallery) return null; // only render first gallery
    const images = b.value.filter(Boolean).map(String).slice(0, 12);
    const prefix = `${galleryPrefix}-${i}`;
    return (
      <section className="gallery-wrap" aria-label="گالری تصاویر" style={{ display: 'flex', justifyContent: 'center' }}>
        <div className="gallery-grid" style={{ maxWidth: '1100px', margin: '0 auto' }}>
          {images.map((src, j) => {
            const id = `lb-${prefix}-${j}`;
            return (
              <div className="gallery-item" key={id} style={{ gridColumn: (j === 0 ? 'span 12' : j % 3 === 0 ? 'span 6' : 'span 3') }}>
                <a href={`#${id}`} aria-label={`نمایش تصویر ${j+1}`}>
                  <img src={src} alt={`gallery-${j+1}`} style={{ width: '100%', height:'100%', objectFit:'cover', display:'block' }} loading="lazy" />
                </a>
                <div id={id} className="lb-modal" aria-hidden>
                  <a href="#" className="lb-backdrop" aria-label="بستن" />
                  <div className="lb-content">
    ...
    if (isYouTube(v)) {
      ...
      return (
        <div className="my-6" style={{ aspectRatio: '16/9' }}>
          <iframe src={embed || ""} title={`video-${i}`} frameBorder="0" allowFullScreen className="w-full h-full" />
        </div>
      );
    }
    return (
